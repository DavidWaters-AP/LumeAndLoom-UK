...
const { data } = await useAsyncGql('getProductCategories', { first: 5 });
...
<template>
  <main>
    ...
    <section class="container my-16" v-if="popularProducts">
      ...
      <ProductRow :products="popularProducts" class="grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-8" />
    </section>

    <section class="container my-16">
      <div class="flex items-end justify-between">
        <h2 class="text-lg tracking-tighter md:text-2xl">{{ $t('messages.shop.shopByCategory') }}</h2>
    ...
    <section class="container grid gap-4 my-24 md:grid-cols-2 lg:grid-cols-4">
      <div class="flex items-center gap-8 p-8 bg-white rounded-lg">
        <img src="/icons/box.svg" class="text-red-500" width="60" height="60" alt="Free Shipping" loading="lazy" />
        <div>
